  ...
  return (
    <div className="flex min-h-screen flex-col bg-gradient-to-b from-[#f7f3ee] via-white to-[#f0ebe4]">
      ...
      <section className="flex-1 flex flex-col items-center justify-center px-6 py-16 md:py-24 text-center relative overflow-hidden">
        {/* Arkaplan: villa silueti */}
        <div className="absolute inset-0 pointer-events-none select-none overflow-hidden">
          <svg className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[1000px] h-auto opacity-[0.055] md:opacity-[0.07]" viewBox="0 0 800 600" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ filter: "blur(1px)" }}>
            {/* Zemin */}
            <rect x="0" y="520" width="800" height="80" fill="#1a2e2a" />
            {/* Villa ana bina */}
            <rect x="180" y="300" width="400" height="220" rx="4" fill="#1a2e2a" />
            {/* Sol kanat */}
            <rect x="120" y="350" width="70" height="170" rx="3" fill="#1a2e2a" />
            {/* Sağ kanat */}
            <rect x="570" y="350" width="120" height="170" rx="3" fill="#1a2e2a" />
            {/* Çatı */}
            <polygon points="140,300 380,160 620,300" fill="#1a2e2a" />
            {/* Sağ kanat çatı */}
            <polygon points="570,350 630,260 690,350" fill="#1a2e2a" />
            {/* Sol kanat çatı */}
            <polygon points="120,350 155,280 190,350" fill="#1a2e2a" />
            {/* Kapı */}
            <rect x="345" y="420" width="70" height="100" rx="35" fill="#f7f3ee" opacity="0.12" />
            {/* Pencereler ana */}
            {[220, 310, 440, 530].map((x) => (
              <rect key={`vw-${x}`} x={x} y="350" width="50" height="55" rx="3" fill="#f7f3ee" opacity="0.1" />
            ))}
            {/* Havuz */}
            <ellipse cx="680" cy="490" rx="70" ry="25" fill="none" stroke="#f7f3ee" strokeWidth="1.5" opacity="0.08" />
            <ellipse cx="680" cy="490" rx="50" ry="18" fill="none" stroke="#f7f3ee" strokeWidth="0.5" opacity="0.05" />
            {/* Palmiye ağaçları */}
            <rect x="95" y="400" width="6" height="120" fill="#1a2e2a" />
            <ellipse cx="98" cy="390" rx="30" ry="35" fill="#1a2e2a" opacity="0.7" />
            <rect x="720" y="410" width="6" height="110" fill="#1a2e2a" />
            <ellipse cx="723" cy="400" rx="28" ry="32" fill="#1a2e2a" opacity="0.7" />
            {/* Yıldızlar */}
            <circle cx="180" cy="60" r="2" fill="#1a2e2a" opacity="0.3" />
            <circle cx="420" cy="35" r="1.5" fill="#1a2e2a" opacity="0.25" />
            <circle cx="600" cy="55" r="2" fill="#1a2e2a" opacity="0.3" />
            <circle cx="700" cy="30" r="1.5" fill="#1a2e2a" opacity="0.2" />
            <circle cx="280" cy="90" r="1" fill="#1a2e2a" opacity="0.2" />
            {/* Ay */}
            <circle cx="600" cy="80" r="20" fill="none" stroke="#f7f3ee" strokeWidth="0.5" opacity="0.06" />
          </svg>
        </div>
        ...
        <div className="max-w-4xl mx-auto space-y-9 relative z-10">
          <div className="inline-flex items-center gap-2 rounded-full bg-white/70 backdrop-blur-sm border border-amber-200/50 px-4 py-1.5 text-xs font-medium text-amber-800 shadow-sm">
            ...
            Villa &amp; Kiralık Yazlık İşletmeleri İçin
          </div>
          ...
          <p className="text-base md:text-lg text-teal-800/70 max-w-xl mx-auto leading-relaxed">
            ...
            yanıtlar. <strong className="text-teal-900 font-semibold">Villa
            kiralama sürecini otomatize eder</strong>, müsaitlik sorgulamadan
            ödemeye kadar her adımı yönetir.
          </p>
      ...
      <section className="px-6 py-20 md:py-28 bg-gradient-to-b from-white/80 via-white to-white/80 relative">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-14">
            <h2 className="text-3xl md:text-4xl font-bold text-[#1a2e2a]">Villa İşletmelerine Özel Çözümler</h2>
            <p className="text-teal-800/60 mt-3 max-w-md mx-auto text-base">Havuzlu villa, dağ evi, sahil konağı — her konsepte uygun dijital resepsiyonist</p>
          </div>
      ...
      <section className="px-6 py-16 md:py-20 bg-white relative">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-teal-800/60 mt-3 max-w-md mx-auto text-base">Villa işletmeniz büyüdükçe planınızı yükseltin</p>
          </div>
          <div className="grid md:grid-cols-3 gap-5">
      ...
      <section className="px-6 py-20 md:py-28 bg-gradient-to-br from-teal-900 via-teal-800 to-teal-700 text-white text-center relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          ...
          <svg className="absolute inset-0 w-full h-full opacity-[0.03]" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
            <defs><pattern id="villa-grid" width="20" height="20" patternUnits="userSpaceOnUse"><path d="M 20 0 L 0 0 0 20" fill="none" stroke="white" strokeWidth="0.5" /></pattern></defs>
            <rect width="100" height="100" fill="url(#villa-grid)" />
          </svg>
        </div>
        <div className="max-w-2xl mx-auto space-y-8 relative z-10">
          <h2 className="text-3xl md:text-5xl font-bold leading-tight">Villanız İçin Dijital Resepsiyonist Zamanı</h2>
...
const villaFeatures = [
  {
    icon: "🏡",
    title: "Müsaitlik & Fiyat Teklifi",
    desc: "Hangi tarihler boş, kaç kişilik, havuz açık mı, evcil hayvan kabul ediliyor mu — tüm soruları anında yanıtlar.",
  },
  {
    icon: "📝",
    title: "Misafir Bilgilerini Otomatik Topla",
    desc: "Rezervasyon sırasında misafir sayısı, giriş saati, özel istekler gibi tüm bilgileri sorar ve kaydeder.",
  },
  {
    icon: "🤝",
    title: "Kapora & Ödeme Yönetimi",
    desc: "Kapora talebini iletir, ödeme linki gönderir, ödeme durumunu takip eder ve onay mesajını atar.",
  },
  {
    icon: "🔐",
    title: "Akıllı Kasa & Giriş Kodları",
    desc: "Check-in günü kapı kodu/kart bilgilerini, wifi şifresini ve ev talimatlarını otomatik iletir.",
  },
  {
    icon: "⭐",
    title: "Sonraki Tatil Hatırlatması",
    desc: "Misafir ayrıldıktan sonra teşekkür mesajı gönderir ve bir sonraki tatil için erken rezervasyon fırsatını hatırlatır.",
  },
  {
    icon: "📱",
    title: "Çoklu Villa Yönetimi",
    desc: "Birden fazla villanız varsa her birine ayrı resepsiyonist atayın, tümünü tek panelden yönetin.",
  },
];

const pricingPlans = [
  { name: "Mini", price: "₺0", features: ["1 villa", "7/24 WhatsApp desteği", "Temel rezervasyon", "E-posta bildirimleri"], cta: "Ücretsiz Başla", popular: false, href: "/signup" },
  { name: "Villa", price: "₺349", features: ["5 villaya kadar", "Kapora & ödeme yönetimi", "Giriş kodu otomasyonu", "WhatsApp + Telegram bildirim", "Misafir takip sistemi"], cta: "14 Gün Ücretsiz Dene", popular: true, href: "/signup" },
  { name: "Plus", price: "₺699", features: ["Sınırsız villa", "API entegrasyonu", "Özel markalı WhatsApp", "VIP destek", "Gelişmiş raporlama"], cta: "İletişime Geç", popular: false, href: "/signup" },
];
